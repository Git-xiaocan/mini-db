...
func testRoseDBZScore(t *testing.T, ioType IOType, mode DataIndexMode) {
	...
	score1, ok1 := db.ZScore(zsetKey, GetKey(0))
	...
	score2, ok2 := db.ZScore(zsetKey, GetKey(0))
	...
	score3, ok3 := db.ZScore(zsetKey, GetKey(0))
	...
	//	_ = db2.Close()
	//}()
	//score4, ok4 := db2.ZScore(zsetKey, GetKey(0))
...
func testRoseDBZRem(t *testing.T, ioType IOType, mode DataIndexMode) {
	...
	c2 := db.ZCard(zsetKey)
	assert.Equal(t, 2, c2)
	_, ok := db.ZScore(zsetKey, GetKey(1))
...
func testRoseDBZRevRange(t *testing.T, ioType IOType, mode DataIndexMode) {
	...
	score, ok := db.ZScore(zsetKey, GetKey(3))
	assert.True(t, ok)
...
func testRoseDBZRank(t *testing.T, ioType IOType, mode DataIndexMode) {
	...
	r1, ok := db.ZRank(zsetKey, GetKey(-1))
	assert.False(t, ok)
	assert.Equal(t, 0, r1)

	r2, ok := db.ZRank(zsetKey, GetKey(3))
	assert.True(t, ok)
	assert.Equal(t, 3, r2)
	r3, ok := db.ZRevRank(zsetKey, GetKey(1))
	assert.True(t, ok)
