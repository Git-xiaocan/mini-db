...
func popInternal(db *minidb.MiniDB, args [][]byte, isLeft bool) (interface{}, error) {
	if len(args) < 1 {
...
func zRank(cli *Client, args [][]byte) (interface{}, error) {
	...
	rank, ok := cli.db.ZRank(args[0], args[1])
	if !ok {
		return nil, nil
	}
	return rank, nil
...
func zRevRank(cli *Client, args [][]byte) (interface{}, error) {
	...
	rank, ok := cli.db.ZRevRank(args[0], args[1])
	if !ok {
		return nil, nil
	}
	return rank, nil
